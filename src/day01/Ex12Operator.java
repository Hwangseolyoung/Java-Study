...
// 산술연산자의 경우, 
// 같은 종류(정수와 정수 혹은 실수와 실수)의 데이터타입을 연산하면 해당 데이터타입이 결과로 나오지만
// 다른 종류(정수와 실수)의 데이터타입을 연산하면 데이터손실이 발생되지 않는 데이터타입이 결과로 나온다.

// 또한 수학에서의 우선순위가 그대로 적용되므로 *, /, %가 +, -보다 먼저 실행이 된다.
